...
async function main(buildDir?: string): Promise<void> {
	...
	if (arch !== 'universal') {
		await spawn('plutil', [
			...
			'-string',
			'An application in CodeeForge wants to use AppleScript.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-replace',
			...
			'-string',
			'An application in CodeeForge wants to use the Microphone.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-replace',
			...
			'-string',
			'An application in CodeeForge wants to use the Camera.',
			`${infoPlistPath}`
		]);
	}
